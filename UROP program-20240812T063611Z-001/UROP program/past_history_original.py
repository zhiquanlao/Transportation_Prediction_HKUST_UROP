...
sensors_short=["339","376","413","467","509"]# change this list to alter the segment of data (remember to include all sensors in between the segment!)
...
for w in weekdays:#for each weekday in the list
    ...
    for date in all_days: #for each day
        ...
        for start_no in range(len(sensors_short)-1):
            for end_no in range(start_no+1,len(sensors_short)): #for each possible combination of sensors in the segment
        ...
        for start_no_3 in range(len(sensors_short)-1):
            for end_no_3 in range(start_no_3+1,len(sensors_short)): #for each combination
        ...
        print(date,"Done!")
        output_df.to_pickle("D:/abc/"+date+"speed_inputs.pickle")
